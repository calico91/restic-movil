...
// Ticket de factura/transacción para impresora de 80mm (48 chars por línea)
class TransactionTicket80mm implements PrintableTicket {
  final TransactionReceiptModel transaction;

  TransactionTicket80mm({required this.transaction});
  ...
  Future<void> printReceipt(ThermalPrinterPort printer) async {
    ...
    String buildDisplayName(String? pName, String? obs) {
      ...
      final String base = pName ?? 'Producto';
    ...
    for (final item in transaction.items ?? []) {
      final String name = buildDisplayName(item.productName, item.observations);
